#include <cstring>
#include <fstream>
#include "sha2563.h"
#include <iostream>
#include <bitset>

using namespace std;
 
const unsigned int SHA256::sha256_k[64] = //UL = uint32
            {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
             0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
             0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
             0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
             0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
             0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
             0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
             0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
             0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
             0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
             0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
             0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
             0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
             0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
             0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
             0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2};
 
void SHA256::rev_transform()
{



    unsigned char *sub_block;
    uint32 i = 0;
    uint32 j = 0;

    pp--;
    sub_block = pmem[pp];
    pmem[pp] = 0; 

    lp--;
    for (i += mem[lp], mem[lp] -= i; i > 0; i--) {

        lp--;
        for (j += mem[lp], mem[lp] -= j; j > 0; j--) {
            m_h[j - 1] -= wv[j - 1];
        }

        lp--;
        for (j += mem[lp], mem[lp] -= j; j > 0; j--) {

            wv[0] -= t1 + t2;
            wv[0] += wv[1];

            wv[1] -= wv[0];
            wv[1] += wv[2];

            wv[2] -= wv[1];
            wv[2] += wv[3];
         
            wv[3] -= wv[2];
            wv[3] += wv[4] - t1;

            wv[4] -= wv[3] + t1;
            wv[4] += wv[5];

            wv[5] -= wv[4];
            wv[5] += wv[6];

            wv[6] -= wv[5];
            wv[6] += wv[7];

            wv[7] -= wv[6];
            pop(&(wv[7]));

            t2 -= SHA256_F1(wv[0]) + SHA2_MAJ(wv[0], wv[1], wv[2]);
            pop(&(t2));

            t1 -= wv[7] + SHA256_F2(wv[4]) + SHA2_CH(wv[4], wv[5], wv[6])
                + sha256_k[j - 1] + w[j - 1];
            pop(&(t1));

        }

        lp--;
        for (j += mem[lp], mem[lp] -= j; j > 0; j--) {

            wv[j - 1] -= m_h[j - 1];
            pop(&(wv[j - 1]));

        }

        lp--;
        for (j += mem[lp], mem[lp] -= j; j > 16; j--) {

            w[j - 1] -=  SHA256_F4(w[j -  2 - 1]) + w[j -  7 - 1] + SHA256_F3(w[j - 15 - 1]) + w[j - 16 - 1];
            pop(&(w[j - 1]));

        }

        for (; j > 0; j--) {

            REV_SHA2_PACK32(&sub_block[j - 1 << 2], &w[j - 1]);
            pop(&(w[j - 1]));

        }

        pp--;
        sub_block = 0;
        sub_block = pmem[pp];
  
    }

}

void SHA256::transform(unsigned char *message, unsigned int block_nb)
{

    unsigned char *sub_block;
    uint32 i;
    uint32 j;
    for (i = 0; i < (int) block_nb; i++) {
        cout << "1 giro ale" << endl;

        pmem[pp] = sub_block;
        sub_block = 0;
        pp++;
        sub_block = message + (i << 6);

        for (j = 0; j < 16; j++) {

            push(&(w[j]));
            SHA2_PACK32(&sub_block[j << 2], &w[j]);

        }
        for (j = 16; j < 64; j++) {

            push(&(w[j]));
            w[j] =  SHA256_F4(w[j -  2]) + w[j -  7] + SHA256_F3(w[j - 15]) + w[j - 16];

        }

        push(&(j));

        for (j = 0; j < 8; j++) {

            push(&(wv[j]));
            wv[j] += m_h[j];
    
        }

        push(&(j));

        for (j = 0; j < 64; j++) {

            push(&(t1));
            t1 += wv[7] + SHA256_F2(wv[4]) + SHA2_CH(wv[4], wv[5], wv[6])
                + sha256_k[j] + w[j];

            push(&(t2));
            t2 += SHA256_F1(wv[0]) + SHA2_MAJ(wv[0], wv[1], wv[2]);
  
            push(&(wv[7]));
            wv[7] += wv[6];
            
  
            wv[6] -= wv[7];
            wv[6] += wv[5];
 
            wv[5] -= wv[6];
            wv[5] += wv[4];
 
            wv[4] -= wv[5];
            wv[4] += wv[3] + t1;

            wv[3] -= wv[4] - t1;
            wv[3] += wv[2];

            wv[2] -= wv[3];
            wv[2] += wv[1];

            wv[1] -= wv[2];
            wv[1] += wv[0];

            wv[0] -= wv[1];
            wv[0] += t1 + t2;

        }

        push(&(j));

        for (j = 0; j < 8; j++) {
            m_h[j] += wv[j];
        }

        push(&(j));
    }

    push(&(i));

    pmem[pp] = sub_block;
    sub_block = 0;
    pp++;
}


 
void SHA256::init() //vengono inizializzate le 8 variabili che sono i numeri primi di non mi ricordo cosa.
{

    m_h[0] = 0x6a09e667;
    m_h[1] = 0xbb67ae85;
    m_h[2] = 0x3c6ef372;
    m_h[3] = 0xa54ff53a;
    m_h[4] = 0x510e527f;
    m_h[5] = 0x9b05688c;
    m_h[6] = 0x1f83d9ab;
    m_h[7] = 0x5be0cd19;
    m_len = 0;
    m_tot_len = 0;
    lp = 0;
    pp = 0;
    sp = 0;
}

void SHA256::update(unsigned char *message, unsigned int len, unsigned char* digest) //i valori sono il mesasggio e la sua lunghezza
{

    unsigned int block_nb = 0;
    unsigned int fm_len = 0;
    unsigned int blocks_n = 0;
    unsigned int i = 0;

    blocks_n += len / SHA224_256_BLOCK_SIZE;
    m_len += len % SHA224_256_BLOCK_SIZE;

    cout << "SHA224_256_BLOCK_SIZE: " << SHA224_256_BLOCK_SIZE << endl;
    cout << "len: " << len << endl;
    cout <<"m_len: " << m_len << endl;
    cout <<"blocks_n: " << blocks_n << endl;

    block_nb += (1 + ((SHA224_256_BLOCK_SIZE - 9)                    
                     < (m_len)));  //block_nb diventa 1 se la lunghezza del messaggio è inferiore a 56 bit, 2 altrimenti
    
    cout <<"block_nb: " << block_nb << endl;

    fm_len += (blocks_n * SHA224_256_BLOCK_SIZE) + (block_nb * SHA224_256_BLOCK_SIZE);
    m_block = (unsigned char*) malloc(sizeof(char) * fm_len);
    for (; i < fm_len; i++) {
        m_block[i] = 0x00;
    }

    memcpy(m_block, message, len);

    m_block[len] += 0x80;

    for (i += len + 1 ; i % SHA224_256_BLOCK_SIZE != SHA224_256_BLOCK_SIZE - 9; i++) {

        m_block[i] += 0x00;

    }

    push(&(i));
    
    SHA2_UNPACK32(len << 3, m_block + fm_len- 4);

    int j;

    for (j=0; j < (blocks_n * SHA224_256_BLOCK_SIZE) + (block_nb * SHA224_256_BLOCK_SIZE); j++) {
        bitset<8> x(m_block[j]);
        cout << x << '\n';
    }
    cout << endl;
    cout << "J: " << j << endl;



    transform(m_block, fm_len/SHA224_256_BLOCK_SIZE);

    for (i = 0 ; i < 8; i++) {
        SHA2_UNPACK32(m_h[i], &digest[i << 2]);
    }

    mem[lp] += i;
    i -= mem[lp];
    lp++;

    push(&(block_nb));

    push(&(fm_len));
    
    push(&(blocks_n));

    cout << "lp: " << lp << endl;
    cout << "sp: " << sp << endl;
    cout << "pp: " << pp << endl;


}

void SHA256::rev_update(unsigned char *message, unsigned int len, unsigned char *digest) //i valori sono il mesasggio e la sua lunghezza
{
    unsigned int block_nb = 0;
    unsigned int fm_len = 0;
    unsigned int blocks_n = 0;
    int i = 0;


    pop(&(blocks_n));

    pop(&(fm_len));

    pop(&(block_nb));

    lp--;

    for (i += mem[lp], mem[lp] -= i ; i > 0; i--) {
        REV_SHA2_UNPACK32(m_h[i], &digest[i << 2]); 
    }

    rev_transform();

    REV_SHA2_UNPACK32(len << 3, m_block + fm_len- 4);

    lp--;

    for (i += mem[lp], mem[lp] -=i ; i >len; i--) {

        m_block[i] -= 0x00;

    }

    m_block[len] -= 0x80;

    memset(m_block, 0, len);

    fm_len -= (blocks_n * SHA224_256_BLOCK_SIZE) + (block_nb * SHA224_256_BLOCK_SIZE);
    
    block_nb -= (1 + ((SHA224_256_BLOCK_SIZE - 9)                    
                     < (m_len)));

    blocks_n -= len / SHA224_256_BLOCK_SIZE;
    m_len -= len % SHA224_256_BLOCK_SIZE;

    for (int j=0; j < 274; j++) {
        bitset<8> x(mem[j]);
        cout << x << '\n';
    }

    cout << "lp: " << lp << endl;
    cout << "sp: " << sp << endl;
    cout << "pp: " << pp << endl;
                 

}

void SHA256::push(unsigned int* a) 
{

    mem[lp] += *a;
    *a -= mem[lp];
    lp++;  

}

void SHA256::pop(unsigned int* a) 
{
    lp--;
    *a += mem[lp];
    mem[lp] -= *a;       

}
 
std::string sha256(std::string input)
{
    
    unsigned char digest[SHA256::DIGEST_SIZE];
    memset(digest,0,SHA256::DIGEST_SIZE);
 
    SHA256 ctx = SHA256();
    ctx.init(); 
    ctx.update( (unsigned char*)input.c_str(), input.length(), digest);

    char buf[2*SHA256::DIGEST_SIZE+1];
    buf[2*SHA256::DIGEST_SIZE] = 0;
    for (int i = 0; i < SHA256::DIGEST_SIZE; i++)
        sprintf(buf+i*2, "%02x", digest[i]);

    cout << "invertiamo!" << endl;    
        
    ctx.rev_update((unsigned char*)input.c_str(), input.length(), digest);    
    return std::string(buf);
}